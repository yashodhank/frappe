...
class BotParser(object):
	...
	def format_list(self, data):
		...
		return ', '.join(['[{name}](#Form/{doctype}/{name})'.format(doctype=self.doctype_names[self.tables[0]],
...
class ListBot(BotParser):
	def get_reply(self):
		if self.startswith('list', 'show'):
			...
			if self.tables:
				return self.format_list(frappe.get_list(self.doctype_names[self.tables[0]]))
...
class CountBot(BotParser):
	def get_reply(self):
		if self.startswith('how many'):
			self.tables = self.reply.identify_tables(self.query.split(None, 1)[1])
			if self.tables:
				return str(frappe.db.sql('select count(*) from `tab{0}`'.format(self.tables[0]))[0][0])
		...
		if self.startswith('find', 'search'):
			...
			if table:
				...
				if self.tables:
					...
					data = frappe.get_list(self.doctype_names[self.tables[0]],
						filters=filters, or_filters=or_filters)
					if data:
						return self.format_list(data)
					else:
						return _("Could not find {0} in {1}").format(text, self.tables[0])
